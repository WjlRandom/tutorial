...
class UploadFile extends React.Component {
  ...
  render() {
    return (
      <div className="upload">
        <div className="upload-title">base64上传图片（本地预览再提交）</div>
  ...
  submit() {
    let preImgList = this.state.preImgList;
    let L = preImgList.length;
    if (L > 0) {
      let formData = new FormData();
      formData.append("fileList", JSON.stringify(preImgList));
      uploadBase(formData).then(response => {
